...
export const assemblyAISpeechMachine = setup({
  ...
  actions: {
    start: async ({ context }) => {
      ...
      type RecordRTCCtor = {
        new (s: MediaStream, o: RecorderOptions): SimpleRecorder
        StereoAudioRecorder: unknown
      }
      const RecordRTCLib = RecordRTC as unknown as RecordRTCCtor

      const recorder: SimpleRecorder = new RecordRTCLib(stream, {
        type: "audio",
